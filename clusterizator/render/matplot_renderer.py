...
class MatPlotRenderer(RendererBase):
    ...
    def render(self, dataset, epoch, clusters_ids, centroids):
        if not self._render_initial_state and epoch == RendererBase.INITIAL: return
        elif not self._render_final_state and epoch == RendererBase.FINAL: return
        elif not self._render_intermediate_state: return
